 ...
 * Stefan Andersson (sand)
       ...
       TODO: can not use arrows in calendar, only responsive to mouse pointer

 ************************************************************************ */

/**
 * A *date field* is like a combo box with the date as popup. As button to open
 * the calendar a calendar icon is shown at the right to the textfield.
 * 
 * To be conform with all form widgets, the {@link qx.ui.form.IForm} interface
 * is implemented.
 * 
 * The following example creates a date field and sets the current date as
 * selected.
 * 
 * <pre class='javascript'>
 * var dateField = new qxe.ui.form.DateField();
 ...
 * </pre>
 * 
 * @childControl list {qxe.ui.control.DateChooser} date chooser component
 * @childControl popup {qx.ui.popup.Popup} popup which shows the list control
 * @childControl textfield {qx.ui.form.TextField} text field for manual date
 *               entry
 * @childControl button {qx.ui.form.Button} button that opens the list control
 */
qx.Class.define("qxe.ui.form.DateField", {
	...
	members : {
		...
		_setDefaultDateFormat : function() {
			this.setDateFormat(qxe.ui.form.DateField.getDefaultDateFormatter());
		},

		/**
		 * Checks for "qx.dynlocale" and adds a listener to the locale changes.
		 * On every change, {@link #_setDefaultDateFormat} is called to
		 * reinitialize the format. You can easily override that method to
		 * prevent that behavior.
		 */
		_addLocaleChangeListener : function() {
			// listen for locale changes
			if (qx.core.Environment.get("qx.dynlocale")) {
				this.__localeListenerId = qx.locale.Manager.getInstance()
						.addListener("changeLocale", function() {
							this._setDefaultDateFormat();
						}, this);
			}
		},
		...
		setValue : function(value) {
			...
			dateChooser.getCalendar().setValue(value);
		},

		/**
		 * Returns the current set date, parsed from the input-field
		 * corresponding to the {@link #dateFormat}. If the given text could
		 * not be parsed, <code>null</code> will be returned.
		 * 
		...
		resetValue : function() {
			...
			// set the date in the datechooser
			var dateChooser = this.getChildControl("list");
			dateChooser.getCalendar().setValue(null);
		},
		...
		_createChildControlImpl : function(id, hash) {
			...
			case "textfield":
				...
				control.addListener("changeValue",
						this._onTextFieldChangeValue, this);
			...
			case "list":
				control = new qxe.ui.control.DateChooser();
				control.setFocusable(false);
			...
			case "popup":
				...
				control.addListener("changeVisibility",
						this._onPopupChangeVisibility, this);
				break;
			}
		 ...
		 * 
		 * @param e
		 *            {qx.event.type.Pointer} The pointer event.
		 */
		_onChangeDate : function(e) {
			var textField = this.getChildControl("textfield");

			var selectedDate = this.getChildControl("list").getCalendar().getValue();
		 ...
		 * 
		 * @param e
		 *            {qx.event.type.Pointer} Pointer tap event
		 */
		_onTap : function(e) {
			this.close();
		},
		 ...
		 * 
		 * @param e
		 *            {qx.event.type.Focus} The blur event.
		 */
		_onBlur : function(e) {
			this.close();
		},
		 ...
		 * the popup. If the list is cloned, all key events will not be
		 * processed further.
		 * 
		 * @param e
		 *            {qx.event.type.KeySequence} Keypress event
		 */
		_onKeyPress : function(e) {
			...
			if (iden === "Left" || iden === "Right" || iden === "Down"
					|| iden === "Up") {
				e.preventDefault();
			}

			// forward the rest of the events to the date chooser
			this.getChildControl("list").getCalendar().handleKeyPress(e);
		},
		 ...
		 * 
		 * @param e
		 *            {qx.event.type.Data} Property change event
		 */
		_onPopupChangeVisibility : function(e) {
			e.getData() == "visible" ? this.addState("popupOpen") : this
					.removeState("popupOpen");
			...
			if (popup.isVisible()) {
				...
				chooser.getCalendar().setValue(date);
			}
		},

		/**
		 * Reacts on value changes of the text field and syncs the value to the
		 * combobox.
		 * 
		 * @param e
		 *            {qx.event.type.Data} Change event
		 */
		_onTextFieldChangeValue : function(e) {
			...
			if (date != null) {
				...
				list.getCalendar().setValue(date);
			}
